...
import util
from SparkIt import LOGGER

DELETE_TIMEOUT = 5


@spark.on(util.admin_cmd(pattern="load (?P<shortname>\w+)$"))  # pylint:disable=E0602
async def load_reload(event):
    await event.delete()
    shortname = event.pattern_match["shortname"]
    try:
        if shortname in spark._plugins:  # pylint:disable=E0602
            spark.remove_plugin(shortname)  # pylint:disable=E0602
        spark.load_plugin(shortname)  # pylint:disable=E0602
...
@spark.on(util.admin_cmd(pattern="(?:unload|remove) (?P<shortname>\w+)$"))  # pylint:disable=E0602
async def remove(event):
    await event.delete()
    shortname = event.pattern_match["shortname"]
    if shortname == "_core":
        msg = await event.respond(f"Not removing {shortname}")
    elif shortname in spark._plugins:  # pylint:disable=E0602
        spark.remove_plugin(shortname)  # pylint:disable=E0602
...
@spark.on(util.admin_cmd(pattern="send plugin (?P<shortname>\w+)$"))  # pylint:disable=E0602
...
@spark.on(util.admin_cmd(pattern="install plugin"))  # pylint:disable=E0602
    ...
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(
                ...
                spark.n_plugin_path  # pylint:disable=E0602
            )
            if "(" not in downloaded_file_name:
                spark.load_plugin_from_file(downloaded_file_name)  # pylint:disable=E0602
